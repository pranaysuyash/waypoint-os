...
export const FrontierDashboard: React.FC<FrontierDashboardProps> = ({
  ...
  return (
    <div className="bento-grid animate-fade-in">
      ...
      <div className="bento-item border-l-4 border-l-[#2ecc71]">
        <div>
          <h3 className="text-xs font-mono text-tertiary mb-1 uppercase tracking-widest">Ghost Concierge</h3>
          <p className="text-2xl font-bold">{ghostActive ? 'ACTIVE' : 'IDLE'}</p>
        </div>
        <div className="flex items-center gap-2 mt-4 text-xs text-secondary">
          <div className={`w-2 h-2 rounded-full ${ghostActive ? 'bg-[#2ecc71] animate-pulse' : 'bg-muted'}`} />
      ...
      <div className={`bento-item ${isAnxious ? 'sentiment-glow-anxious' : 'sentiment-glow-calm'}`}>
        <div>
          <h3 className="text-xs font-mono text-tertiary mb-1 uppercase tracking-widest">Traveler Sentiment</h3>
          <p className="text-2xl font-bold">{(sentiment * 100).toFixed(0)}%</p>
        </div>
      ...
      <div className="bento-item col-span-1 md:col-span-2">
        <h3 className="text-xs font-mono text-tertiary mb-2 uppercase tracking-widest">Federated Intelligence Pool</h3>
        <div className="space-y-3">
          {intelHits.length > 0 ? intelHits.map((hit, i) => (
            <div key={i} className="flex gap-3 items-start p-2 bg-lg-glass-bg rounded-lg border border-lg-glass-border">
              ...
              <div>
                <p className="text-sm font-medium">{hit.message || hit.type}</p>
                <p className="text-xs text-muted">Source: Federated Node #412</p>
              </div>
            </div>
          )) : (
            <p className="text-sm text-secondary italic">No active risks detected in this sector.</p>
          )}
        </div>
      </div>
      ...
      <div className="trust-anchor col-span-1 md:col-span-3">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-[10px] font-mono bg-accent-blue/10 text-accent-blue px-2 py-0.5 rounded">AUTHENTICATED LOGIC</span>
        </div>
