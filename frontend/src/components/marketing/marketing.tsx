...
export function BulletList({ items }: { items: string[] }) {
  return (
    <div className='grid gap-3'>
      {items.map((item) => (
        <div key={item} className='flex items-start gap-3 rounded-[18px] bg-[#161b22] px-4 py-3'>
          <span className='mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#3fb950]/10 text-[#3fb950]'>
            <Check className='h-3 w-3' />
          </span>
          <span className='text-[14px] leading-6 text-[#c9d1d9]'>{item}</span>
        </div>
      ))}
    </div>
  );
