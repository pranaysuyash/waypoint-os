 ...
 *
 * Run with: npx tsx tools/validate-contrast.ts
 */

import { validateTokenColors } from "../src/lib/contrast-utils.js";
...
for (const result of results) {
  ...
  console.log(
    `${icon} ${status.padEnd(4)} | ${result.name.padEnd(35)} | ${result.ratio.toFixed(2)}:1`
  );
}
