"""
tools/memory_slot_review.py — Shadow-window review for E-D slot 1 (memory
...
Pre-registered criteria (mirrored in Docs/architecture/MEMORY_SLOT_WIRING_2026-09-15.md
...
The tool NEVER flips the mode. Verdict REVIEW_READY means: criteria hold,
...
Usage:
    .venv/bin/python tools/memory_slot_review.py                  # human summary
    .venv/bin/python tools/memory_slot_review.py --json           # machine-readable
    .venv/bin/python tools/memory_slot_review.py --agency <id> --since 2026-09-15
"""
...
def load_events(audit_file: Path, agency: Optional[str], since: Optional[str]) -> List[Dict[str, Any]]:
    """Reads promotion events from the JSONL audit log, newest last."""
...
def review(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for event in events:
        ...
        for promo in promoted:
            ...
            memory_id = rationale.split(":")[-1] if rationale else "unknown"
    ...
    if asks < MIN_ASKS:
        ...
        if top_share is not None and top_share > MAX_SINGLE_MEMORY_SHARE:
            verdict = "GUARDRAIL_VIOLATION"
            reasons.append(
                f"single memory drives {top_share:.0%} of promotions > {MAX_SINGLE_MEMORY_SHARE:.0%} (G3)"
            )
        ...
        if verdict != "GUARDRAIL_VIOLATION":
