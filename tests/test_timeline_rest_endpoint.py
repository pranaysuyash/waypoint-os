...
def test_timeline_endpoint_empty_when_no_file(client):
    """Test that endpoint returns empty events for nonexistent trip."""
    response = client.get("/api/trips/nonexistent-trip-12345/timeline")
...
def test_timeline_endpoint_response_structure(client):
    """Test that the endpoint returns the correct response structure."""
    response = client.get("/api/trips/any-trip-id/timeline")
...
def test_timeline_endpoint_accepts_stage_filter(client):
    """Test that endpoint accepts stage query parameter."""
    response = client.get("/api/trips/test-trip/timeline?stage=decision")
...
def test_timeline_endpoint_valid_json(client):
    """Test that the endpoint returns valid JSON."""
    response = client.get("/api/trips/test-trip-json-check/timeline")
