...
Date: 2026-04-15 09:48:44 IST  
Status: Specified for full product surface (implementation can phase; scope is not reduced)
...
- Streamlit (`apps/streamlit_workbench.py`)

Future convergence:
- Migrate high-value panels into productized operator app once contracts are stable.
...
## 9) Technical Frontend Architecture (Target)

Recommended productized stack (post-Streamlit):
- Next.js App Router
- TypeScript
...
Wave 1:
- Internal workbench complete (Streamlit)
...
1. Internal-first and productized-next are both part of one plan; no scope rollback.
2. Workbench is not throwaway; it is contract-validation infrastructure.
