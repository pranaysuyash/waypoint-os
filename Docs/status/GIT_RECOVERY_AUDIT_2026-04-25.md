...
| `frontend/src/lib/route-map.ts` | Partially rejected, comment corrected | Stash tried adding catch-all mappings for routes that are frontend-local or explicit. That is not better under the no-duplicate-route rule. Kept deny-by-default documentation instead. |
