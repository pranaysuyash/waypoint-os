...
| 2.5 | R-15 | Make `privacy_guard.check_trip_data()` **log an audit event** at minimum when it no-ops outside dogfood (`privacy_guard.py:484-486`) | Silent no-op is impossible | PII guard | ✅ **DONE 2026-08-31** — safe config now runs a non-blocking Layer 1 `AUDIT` scan + logs; see `R-15_PII_GUARD_DEFAULT_2026-08-31.md` |
| 2.6 | R-15 | Reconcile the disagreement: Layer 2 (SpaCy) was thought fail-closed in prod (`:412-428`) while the gate was fail-open — **the fail-closed branch was unreachable dead code** | One PII posture, explicitly chosen | PII guard | ✅ **DONE 2026-08-31** — posture reconciled at the gate (dogfood+prod-plaintext fail-closed; prod-SQL/beta fail-open-audited); Layer 2 consistently fail-open. ADR: `R-15_PII_GUARD_DEFAULT_2026-08-31.md` |
