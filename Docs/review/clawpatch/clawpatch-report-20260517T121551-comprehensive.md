...
## low: 187MB data/ directory copied into production Docker image with dev-only content

id: fnd_sig-feat-deployment-infrastructu_14ed8ab56c
category: build-release
confidence: medium
triage: risk
status: open
...
The data/ directory (187MB) is copied wholesale into the Docker image. It includes runtime-needed cities.json (4.1MB) but also dev-only subdirectories: fixtures/, drafts/, evals/, audit/, decision_cache/, and accumulated_cities.json with its lock file. The .dockerignore excludes tests/ and Docs/ but not these data subdirectories. This bloats the image by ~180MB with content never used in production.

recommendation:
Add to .dockerignore: `data/fixtures/`, `data/drafts/`, `data/evals/`, `data/audit/`, `data/decision_cache/`, `data/accumulated_cities.*`, `data/logs/`, `data/product_b_events/`. Only cities.json and cities5000.txt (and any runtime config) should be in the production image.

test analysis:
No CI step checks Docker image size or validates that dev-only data is excluded.

suggested regression test:
Add a CI check: `docker run --rm spine-api test ! -d /app/data/fixtures` to verify dev-only data is excluded from the production image.

minimum fix scope:
.dockerignore file — add data/ exclusions for dev-only subdirectories

repro:
1. Build the Docker image.
2. `docker run --rm spine-api du -sh /app/data/` — will report ~187MB.
3. `docker run --rm spine-api ls /app/data/fixtures/` — dev fixtures are present in production.

## medium: Hardcoded --workers 4 in ENTRYPOINT ignores SPINE_API_WORKERS env var

id: fnd_sig-feat-deployment-infrastructu_17f143ff30
category: bug
confidence: high
triage: confirmed-bug
status: open
...
The ENTRYPOINT hardcodes --workers 4 as a CLI argument, which takes precedence over the SPINE_API_WORKERS environment variable. Similarly, --host and --port are hardcoded while their corresponding env vars SPINE_API_HOST and SPINE_API_PORT are defined but never consumed. Any operator attempting to tune worker count via environment variables will find their changes silently ignored.

recommendation:
Replace the exec-form ENTRYPOINT with a shell script entrypoint that reads env vars, or use a CMD that references them: `CMD ["sh", "-c", "uvicorn spine_api.server:app --host ${SPINE_API_HOST} --port ${SPINE_API_PORT} --workers ${SPINE_API_WORKERS}"]`. Alternatively, create an entrypoint.sh script.

test analysis:
No integration test or CI step verifies that the Docker container honors env var overrides. The container currently always starts with defaults.

suggested regression test:
Add a test: `docker run -e SPINE_API_WORKERS=2 spine-api sh -c "ps aux | grep uvicorn"` to verify worker count matches the env var.

minimum fix scope:
Dockerfile ENTRYPOINT/CMD section

repro:
1. Run the container with `-e SPINE_API_WORKERS=8`
2. Observe that uvicorn still starts with 4 workers because the CLI flag overrides the env var.
...
## medium: uv sync --frozen installs dev dependencies into production image

id: fnd_sig-feat-deployment-infrastructu_6fb898d473
category: security
confidence: medium
triage: risk
status: open
...
uv sync --frozen without --no-dev installs all dependency groups including the dev group (pytest, httpx, ruff, aiosqlite). These packages bloat the production image and increase attack surface (httpx provides an HTTP client, pytest can execute arbitrary test code). Production containers should only contain runtime dependencies.

recommendation:
Change line 41 to `RUN uv sync --frozen --no-dev` to exclude dev dependencies from the production venv. Alternatively, use `--only-group main` if the project adopts uv dependency groups for production.

test analysis:
No integration test validates the contents of the production Docker image's installed packages.

suggested regression test:
Add a CI check: `docker run --rm spine-api python -c "import pytest; exit(1)" 2>/dev/null && echo PASS || echo FAIL` — production image should not have pytest.
...
repro:
1. Build the Docker image.
2. Run `docker run --rm spine-api pip list | grep pytest` — pytest will be installed in the production venv.
...
## medium: Hardcoded database password in docker-compose.yml committed to version control

id: fnd_sig-feat-deployment-infrastructu_8f140c31e7
category: security
confidence: high
triage: confirmed-bug
status: open
...
The PostgreSQL password is hardcoded in plaintext in docker-compose.yml, which is committed to version control. While this is a development compose file, it establishes a pattern of embedding credentials in VCS-tracked files. If this compose file is ever used as a template for production or staging, the credential leaks. Additionally, the password is weak (dictionary word + _dev_password).

recommendation:
Use Docker secrets or .env file references: `POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-waypoint_dev_password}` with the actual password in a .env file that is gitignored. The .env file already exists at the project root and is not tracked by git.

test analysis:
No secret scanning or pre-commit hook checks for hardcoded credentials in docker-compose.yml.

suggested regression test:
Add a git pre-commit hook or CI step that detects hardcoded passwords in YAML files (e.g., regex for POSTGRES_PASSWORD: [^$].*[^_password]).

minimum fix scope:
docker-compose.yml environment section

repro:
N/A — this is a configuration hygiene issue, not a runtime bug.

## medium: build-essential in runtime image increases attack surface unnecessarily

id: fnd_sig-feat-deployment-infrastructu_d5f241b597
category: security
confidence: high
triage: confirmed-bug
status: open
...
The runtime stage inherits from base, which installs build-essential (gcc, g++, make). These compilers are only needed during dependency installation (the deps stage) and should not be present in the production image. An attacker gaining code execution in the container could use gcc/make to compile privilege-escalation tools locally. The deps stage already correctly separates dependency building from the runtime image, but because both inherit from base, the runtime image still carries the toolchain.

recommendation:
Split the base stage: create a `build-deps` stage from `python:3.13-slim` that installs build-essential and curl, use it only for the deps stage. Create a separate `runtime-base` from `python:3.13-slim` that installs only curl (needed for HEALTHCHECK). Then `FROM runtime-base AS runtime`.

test analysis:
No container security scanning or image size regression test exists in the project.

suggested regression test:
Add a CI check: `docker run --rm spine-api sh -c '! which gcc || exit 1'` to verify build tools are absent from the runtime image.

minimum fix scope:
Dockerfile — restructure base/deps/runtime stages to isolate build dependencies

repro:
1. Build the Docker image.
2. Run `docker run --rm spine-api dpkg -l build-essential` — it will show the package installed.
3. `gcc` and `make` are available inside the production container.
...
## critical: Dockerfile ENTRYPOINT references non-importable module spine-api (hyphen) — container cannot start

id: fnd_sig-feat-deployment-infrastructu_ed33e12865
category: bug
confidence: high
triage: confirmed-bug
status: open
...
Python module names cannot contain hyphens. The actual package directory on disk is spine_api/ (underscore), confirmed by filesystem inspection. The ENTRYPOINT uses spine-api.server:app which will cause an ImportError at container startup. Additionally, the COPY directive references a non-existent spine-api/ directory — the Docker build would fail before even reaching the runtime stage. The render.yaml correctly uses spine_api.server:app, confirming the underscore form is canonical.

recommendation:
Change COPY target and ENTRYPOINT to use underscores matching the actual package name: `COPY spine_api/ ./spine_api/` and `ENTRYPOINT ["uvicorn", "spine_api.server:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "4"]`. This aligns with render.yaml which already uses the correct form.

test analysis:
No CI pipeline builds the Docker image. The render.yaml uses the correct module path and works, masking the Dockerfile bug for Fly.io deployments that bypass the ENTRYPOINT via release_command.

suggested regression test:
Add a CI step: `docker build -t spine-api-test . && docker run --rm spine-api-test python -c "import spine_api.server"` to verify the container builds and the module is importable.

minimum fix scope:
Dockerfile lines 63 and 79

repro:
1. Run `docker build -t spine-api .` — the COPY spine-api/ step will fail because the directory does not exist.
2. Even if a symlink or renamed directory were used, the Python import `spine-api.server` would fail because hyphens are illegal in Python identifiers.
...
## medium: No security headers configured in Next.js production config or middleware

id: fnd_sig-feat-frontend-webapp-3d8186d_ef85bfb0ae
category: security
confidence: high
triage: confirmed-bug
status: open
feature: Frontend web application (feat_frontend_webapp)

evidence:
- frontend/next.config.ts:1-40
- frontend/next.config.ts:1

The next.config.ts disables the X-Powered-By header but does not configure any security response headers (Content-Security-Policy, X-Frame-Options, X-Content-Type-Options, Strict-Transport-Security, Referrer-Policy). There is no middleware.ts file in the project to add them either. The app handles authentication (login, join routes), API proxying (proxy-core.ts), and cookie parsing (set-cookie-parser) — all of which are common targets for clickjacking, MIME-sniffing, and injection attacks that security headers mitigate. Without CSP in particular, the app has no browser-enforced restriction on script sources or frame ancestors.

recommendation:
Add a headers() export in next.config.ts or create a middleware.ts to set Content-Security-Policy, X-Frame-Options (DENY or SAMEORIGIN), X-Content-Type-Options (nosniff), Referrer-Policy (strict-origin-when-cross-origin), and Permissions-Policy. For HSTS, also configure it at the reverse-proxy/CDN level. Given this is an auth-handling app, a restrictive CSP is especially important.

test analysis:
Security headers are a deployment/configuration concern, not a runtime code path. No existing test inspects HTTP response headers.

suggested regression test:
Integration test that makes a request to the running app and asserts presence and values of security headers (CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy).

minimum fix scope:
Add headers() export to next.config.ts or create frontend/middleware.ts with security header middleware.

## low: gsap imported statically in client components, not in optimizePackageImports

id: fnd_sig-feat-frontend-webapp-6234b58_982aec6871
...
evidence:
- frontend/src/components/marketing/GsapInitializer.tsx:1-5
- frontend/src/app/(traveler)/itinerary-checker/PageClient.tsx:5
- frontend/next.config.ts:15-17

gsap is imported as a static ES module in two client components (GsapInitializer.tsx and the 2448-line PageClient.tsx). Unlike recharts (which uses dynamic(() => import(...)) with ssr: false) and pdfjs-dist/tesseract.js (which use await import() at call time), gsap is bundled unconditionally into the client chunk. gsap-core + ScrollTrigger is ~70KB minified. Adding gsap to optimizePackageImports would allow Next.js to tree-shake unused gsap exports and reduce initial bundle weight. Additionally, recharts and @tanstack/react-table are also tree-shakeable heavy packages absent from the optimization list.

recommendation:
1) Add 'gsap', 'recharts', and '@tanstack/react-table' to the optimizePackageImports array in next.config.ts. 2) Consider lazy-loading gsap via dynamic() or React.lazy() in GsapInitializer since it only runs client-side after mount (useEffect). 3) This is low severity because the app already correctly lazy-loads recharts and pdfjs/tesseract.

test analysis:
Bundle size regressions are not covered by unit tests; they require bundle analysis tooling.

suggested regression test:
Add a CI step that runs ANALYZE=true next build and asserts the gsap chunk size stays below a threshold, or use @next/bundle-analyzer in a smoke test.

minimum fix scope:
Add packages to optimizePackageImports array in frontend/next.config.ts; optionally convert gsap imports to dynamic.

## low: Feature definition has empty tests array despite 126+ test files in the project

id: fnd_sig-feat-frontend-webapp-939878f_9b63c26e05
category: test-gap
confidence: high
triage: test-gap
status: open
...
- frontend/src/__tests__/proxy.test.ts:1
- frontend/src/lib/__tests__/bff-auth.test.ts:1

The clawpatch feature definition lists tests: [] even though the frontend project contains approximately 126 test files spanning lib, components, app routes, and contexts. This means the feature's review coverage is incomplete — clawpatch cannot validate that the frontend's critical paths (auth flows, proxy/BFF layer, trip management) have test coverage because the feature does not link them. This is a metadata gap in the feature definition rather than a missing-test bug per se.

recommendation:
Update the feature definition's tests array to include key test files that validate core frontend behavior: proxy.test.ts, bff-auth.test.ts, api-client.auth.test.ts, contract-surface tests, and representative page/component tests.

test analysis:
This is a feature-definition metadata gap, not a missing test. The tests exist; they are just not linked to the feature for review tooling.

minimum fix scope:
Update the clawpatch feature YAML/JSON to link existing test files.
