...
| F-19 | I | ~~Full-suite phantom failures~~ **ROOT CAUSES FOUND + FIXED 2026-08-31** (three independent mechanisms): (1) module-level `SPINE_API_DISABLE_AUTH=1` in two new test files — pytest imports every module at collection, disabling auth process-wide → auth-middleware 401 tests failed; **fixed** (scoped to autouse monkeypatch fixtures). (2) Runner arg-mode silently dropped CI `--ignore`s + `tests/` target; **fixed** (args now append). (3) Live-server contention (integration tests vs shared DB) — runner warns; excluded via `-m 'not integration'` for baselines. Bonus: the NullPool diagnostic experiment exposed a **real production bug** — commit() releases the connection, post-commit refresh re-checks out without RLS context and fails under FORCE RLS on pool recycle/churn; **fixed** (flush→refresh→commit in `collection_service` + `document_service` ×4). Suite: 40→98(experiment)→**2 residual order-flakes** (both pass in isolation; one in the parallel agent's actively-staged `public_checker.py` — slowapi rate-limit injection signature; carve-out per simultaneous-edit rule) | Residuals re-verify after parallel agent's commit; integration default-exclude still worth doing | P2 |
